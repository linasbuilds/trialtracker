      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 max-w-md w-full text-center">
        ...
        {!status.includes("wrong") && (
          <div className="mt-6 flex justify-center">
            <div className="w-8 h-8 border-4 border-[#1A1A2E] border-t-transparent rounded-full animate-spin"></div>
          </div>
        )}
        {status.includes("wrong") && (
          <a
            href="/login"
            className="mt-6 inline-block bg-[#1A1A2E] text-white px-6 py-2 rounded-lg font-medium hover:opacity-90"
          >
            Go to Login
          </a>
        )}
